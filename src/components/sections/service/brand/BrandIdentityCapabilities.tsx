...
const CapabilityCard = ({ item, idx, total }: { item: any; idx: number; total: number }) => {
  ...
  return (
    <div ref={targetRef} className="w-full relative" style={{ zIndex: idx + 1 }}>
      <motion.div
        className="sticky top-[15vh] group bg-white dark:bg-[#12151C] p-8 sm:p-10 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-2xl flex flex-col sm:flex-row gap-6 sm:gap-8 items-start origin-top"
        style={{
          scale,
          opacity,
          y,
      ...
      >
        <div className="w-16 h-16 shrink-0 bg-indigo-50 dark:bg-[#0B0E14] text-indigo-600 dark:text-[#818cf8] rounded-2xl flex items-center justify-center group-hover:bg-indigo-600 dark:group-hover:bg-[#818cf8] group-hover:text-white dark:group-hover:text-[#1e1b4b] group-hover:-rotate-3 transition-all duration-300 shadow-sm">
          <span className="material-symbols-outlined text-[32px]">{item.icon}</span>
        </div>
        <div className="flex-1 pt-1">
          <h3 className="font-heading-lg text-xl sm:text-2xl font-bold text-slate-900 dark:text-[#f3f4f6] mb-3 group-hover:text-indigo-600 dark:group-hover:text-[#818cf8] transition-colors">
            {item.title}
          </h3>
          <p className="text-slate-600 dark:text-[#9ca3af] text-base sm:text-lg leading-relaxed max-w-2xl">
            {item.desc}
          </p>
        </div>
      </motion.div>
...
export function BrandIdentityCapabilities() {
  ...
  return (
    <section className="py-space-32 bg-slate-50/70 dark:bg-[#0B0E14] px-6 md:px-8 transition-colors duration-300">
