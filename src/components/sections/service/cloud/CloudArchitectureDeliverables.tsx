  ...
  return (
    <section className="py-space-32 bg-slate-50/80 dark:bg-[#050505] border-y border-slate-200/80 dark:border-[#262626] overflow-hidden transition-colors duration-300">
      <div className="px-6 md:px-8 max-w-[1280px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        >
          <h2 className="font-display-lg text-2xl sm:text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-[#fafafa] mb-space-8">
          ...
          <ul className="space-y-6">
            {deliverables.map((item, idx) => (
              ...
              >
                <span
                  className="material-symbols-outlined text-indigo-600 dark:text-[#c084fc] mt-1 text-[24px] group-hover:scale-110 transition-transform"
                  style={{ fontVariationSettings: "'FILL' 1" }}
                >
                  check_circle
                </span>
                <div>
                  <span className="font-bold text-slate-900 dark:text-[#fafafa] text-base sm:text-lg block mb-0.5">
                    {item.title}
                  </span>
                  <span className="text-slate-600 dark:text-[#a3a3a3] text-sm">{item.desc}</span>
                </div>
              </motion.li>
            ))}
          </ul>
