  ...
  return (
    <section className="py-space-32 bg-slate-900 text-white overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-6 md:px-8 grid lg:grid-cols-12 gap-12 items-start">
        ...
        >
          <span className="text-indigo-400 font-mono text-xs uppercase tracking-widest font-bold block mb-3">
            Methodology
          </span>
          <h2 className="font-display-lg text-2xl sm:text-3xl md:text-4xl font-extrabold mb-6 leading-tight">
            Our Engineering Process
          </h2>
          <p className="text-slate-300 text-base sm:text-lg leading-relaxed max-w-md">
            Performance isn't a one-time fix; it's a rigorous process of measurement, remediation,
            and maintenance.
          </p>
        </motion.div>
          ...
          <div className="space-y-10">
            {steps.map((p, idx) => (
              ...
              >
                ...
                <div>
                  <span className="font-mono text-xs font-bold text-indigo-400 mb-1 block group-hover:text-indigo-300 transition-colors">
                    STEP {p.num}
                  </span>
                  <h3 className="font-bold text-xl sm:text-2xl text-white mb-2 group-hover:text-indigo-300 transition-colors">
                    {p.title}
                  </h3>
