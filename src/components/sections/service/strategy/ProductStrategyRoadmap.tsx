  ...
  return (
    <section className="py-space-24 px-6 md:px-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row gap-8 lg:gap-12 items-start">
        ...
        >
          <h2 className="font-display-lg text-2xl sm:text-3xl md:text-4xl font-extrabold text-slate-900">The Nexus Roadmap Process</h2>
          <p className="text-slate-600 mt-space-4 text-sm sm:text-base leading-relaxed">A proven 4-phase methodology designed to turn concepts into market-ready leaders.</p>
        </motion.div>
        ...
        <div className="md:w-2/3 space-y-6 relative" ref={sectionRef}>
          ...
          <div 
            ...
            style={{ background: `linear-gradient(to bottom, #4f46e5 ${scrollProgress}%, #e2e8f0 ${scrollProgress}%)` }}
          ></div>
          ...
          {phases.map((phase, idx) => (
            ...
            >
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold shrink-0 transition-colors duration-300 shadow-md ${scrollProgress >= phase.thresh ? 'bg-indigo-600 text-white scale-105' : 'bg-slate-100 text-slate-600'}`}>
                {phase.num}
              </div>
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {phase.tags.map((tag, tIdx) => (
                    <span key={tIdx} className="bg-slate-100 px-3 py-1 rounded-md text-xs font-semibold text-slate-700">
                      {tag}
                    </span>
                  ))}
                </div>
